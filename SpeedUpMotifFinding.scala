...
object KMP {
    def while0[T](cond: T => Boolean)(f: T => T, j: T): T = 
    ...
    def preprocess(p: String): List[Int] = {
        def loop(s:String, j: Int, b: List[Int]): List[Int] = s match {
            case "" => b.reverse
            case _ => {
                val j1 = while0[Int](x => x>=0 && s.head!=p(x))(x => b(b.size-x-1), j) + 1
...
//                println("%d %d : %s".format(i, j, report.mkString(" ")))
                val j1 = while0[Int](x => x>=0 && t(i)!=p(x))(x => b(x), j) + 1
                if (j1==p.length) loop(i+1, b(j1), (i-j)::report) 
